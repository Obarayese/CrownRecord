...
function pickMimeType() {
  ...
  if (wantMp4) {
    ...
    if (mp4) return mp4;
    setStatus('MP4 encoding not supported here ΓÇö recording as WebM instead.', '');
  }
  ...
  if (previewHint) {
    if (isRecording) {
      ...
      previewHint.textContent = cam
        ? 'Live preview ΓÇö screen + webcam bubble (saved to your file).'
        : 'Live preview ΓÇö screen capture (saved to your file).';
    }
  }
}
...
function formatCaptureStatus(stream, audioNote) {
  ...
  return `Capturing ${w}├ù${h} @ ${fps} ┬╖ VP9 ${VIDEO_BITRATE / 1_000_000}Mbps${audioNote}`;
}
...
async function captureDesktopStream(sourceId, includeSystemAudio) {
  ...
  try {
    ...
    setStatus(
      `System audio unavailable for this source ΓÇö video only. (${err.message})`,
      'error',
    );
    return navigator.mediaDevices.getUserMedia({ audio: false, video });
  }
...
async function startCamera() {
  ...
  try {
    ...
    setStatus(
      `Camera on ┬╖ ${label} (${settings.width || '?'}├ù${settings.height || '?'})`,
      'ok',
    );
...
async function loadSources() {
  ...
  sourceSelect.innerHTML = '<option value="">LoadingΓÇª</option>';
  ...
  try {
    ...
    setStatus(
      `${sources.length} sources ready. Γÿà = Zoom, Meet, Teams, etc. ΓÇö pick the meeting window.`,
    );
...
async function initSupportUi() {
  ...
  openLogsBtn.addEventListener('click', async () => {
    try {
      ...
      setStatus('Log folder opened ΓÇö attach latest .log to support email.', 'ok');
    } catch (err) {
...
if (exportFormat) {
  exportFormat.addEventListener('change', () => {
    ...
    if (exportFormat.value === 'mp4' && !mp4Ok) {
      setStatus('MP4 may not be available on this PC ΓÇö WebM will be used if needed.', '');
    }
  });
}
